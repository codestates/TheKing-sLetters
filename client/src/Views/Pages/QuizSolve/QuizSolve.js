...
const initialQuiz = {
  ...
  quizContents: {
    image_url:
      'https://img2.daumcdn.net/thumb/R658x0.q70/?fname=https://t1.daumcdn.net/news/202105/25/thequizlive/20210525061018472jdob.jpg',
  },
  answerType: '이미지 답안',
  answerContents: [
...
const QuizSolve = ({ quizId = QUIZ_ID_FOR_TEST }) => {
  ...
  useEffect(() => {
    const initialFetchUserData = async () => {
      try {
        ...
        const refinedUserInfo = await importRefinedFetchData(
          'user',
          rawUserInfo
        );
  ...
  useEffect(() => {
    const initialFetchQuizDate = async () => {
      try {
        ...
        const refinedQuizData = await importRefinedFetchData(
          'quiz',
          rawQuizData
        );
  ...
  const PageErrorMsgDisplay = () => {
    ...
    if (errorList.loginError) {
      error.push(
        <p key="login_error_msg" className="login_error_msg">
    ...
    if (errorList.quizError) {
      error.push(
        <p key="quiz_error_msg" className="quiz_error_msg">
      ...
      {!pageIsLoading ? (
        <>
          ...
          {doneSubmit ? (
            <CheckAnswer
              quizData={quizData}
              isCorrect={isCorrectAnswer.current}
            ></CheckAnswer>
          ) : null}
          {commentIsLoading ? (
            <img
